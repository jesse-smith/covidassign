...
approx_join <- function(
  ...
  distance_col <- ifelse(
    ...
    NULL,
    vctrs::vec_assert(distance_col, ptype = character())[[1L]]
  )
  ...
  join_fn <- switch(
    ...
    rlang::abort("`na_matches` must be 'na', 'never', or 'always'")
  )
...
approx_join_na_always <- function(x, y, by, max_dist, mode, ...) {
  ...
  x_id <- dplyr::mutate(
    x,
    .id_temp_ = vec_seq_along(x),
    .all_na_ = dplyr::c_across({{ by }}) %>% is.na() %>% all()
  )
  y_id <- dplyr::mutate(
    y,
    .id_temp_ = vec_seq_along(y),
    .all_na_ = dplyr::c_across({{ by }}) %>% is.na() %>% all()
  )
  remove(x, y)
  ...
  x_drop_empty %>%
    ...
    dplyr::summarize(join_to_y(dplyr::c_across("data")))
}

replace_na_chr <- function(.data, na_chr = "NA_character_", invert = FALSE) {
  ...
  # Create mapping function for `across()`
  map_na_chr <- purrr::partial(
    stringr::str_replace,
    pattern = !!ifelse(invert, na_chr, NA_character_),
    replacement = !!ifelse(invert, na_chr, "NA_character_")
  )

  # Replace missing values
  dplyr::mutate(.data, dplyr::across(where(is.character), map_na_chr))
}
